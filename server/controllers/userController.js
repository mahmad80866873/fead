...
export async function createUser(req, res) {
  try {
    const { matricule, password, nom, prenom, role, service } = req.body
    if (!matricule || !password) return res.status(400).json({ error: 'Matricule et mot de passe requis.' })
    const user = await User.create({ matricule, password, nom, prenom, role, service, actif: true })
...
export async function updateUser(req, res) {
  try {
    const { nom, prenom, role, service, actif, password } = req.body
